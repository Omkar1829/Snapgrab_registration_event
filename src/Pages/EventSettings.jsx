...
const EventSettings = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await axios.put(
        `${config.apiUrl}/events/${selectedEvent.ID}`,
        payload
      );
  ...
  const handleAddBooth = async () => {
    ...
    try {
      await axios.post(`${config.apiUrl}/activity/create`, payload);

      console.log(selectedEvent, "ID");

      const res = await axios.get(
        `${config.apiUrl}/activity?EventId=${selectedEvent.EventId}`
      );
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await axios.get(
        `${config.apiUrl}/qrregistration/csv-template`,
        {
          responseType: "blob",
        }
      );
